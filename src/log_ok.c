...
void log_ok(GtkWidget *widget, t_wid *wid) {
	(void)widget;
	wid->log->logname = (char *)gtk_entry_get_text 
		(GTK_ENTRY (wid->log->log_name));
	wid->log->logpas = (char *)gtk_entry_get_text 
		(GTK_ENTRY (wid->log->log_pas));
	
	if (mx_strcmp(wid->log->logname, "\0") == 0 || 
		mx_strcmp(wid->log->logpas, "\0") == 0 )
