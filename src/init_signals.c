...
static void first_choise(t_wid *wid) {
    ...
    //выбор ховером
	g_signal_connect(wid->sig->sig_gif, "enter_notify_event", 
...
static void result(t_wid *wid) {
	...
	g_signal_connect(wid->log->log_but, "clicked", 
		G_CALLBACK(log_ok), wid);
	g_signal_connect(wid->sig->sig_but, "clicked", 
		G_CALLBACK(sig_ok), wid);
	g_signal_connect(wid->sig->sig_photo, "clicked", 
		G_CALLBACK(add_sig_photo), wid);
}
...
void init_signals_start(t_wid *wid, t_eye *eye) {
    ...
    first_choise(wid);
    result(wid);
    g_signal_connect(wid->log->log_pas, "icon_press", 
		G_CALLBACK(make_vis), eye);
	g_signal_connect(wid->sig->sig_pas, "icon_press", 
		G_CALLBACK(make_vis1), eye);
	g_signal_connect(wid->sig->sig_pas2, "icon_press", 
		G_CALLBACK(make_vis2), eye);
	g_signal_connect(wid->badact_but, "clicked", 
		G_CALLBACK(hide_bad), wid);
}
